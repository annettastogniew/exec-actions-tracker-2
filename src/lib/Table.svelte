<script>
    ...
    const filterData = (action) => {
        ...
        const categoryAndPrimaryTag = filters
            .map((col) => filter[col].includes(action[col]))
            .every(Boolean);
    ...
    const selectedActions = $derived(
        allActions["data"].filter((action) => filterData(action)),
    );
...
<main>
    ...
    {:else}
        <p class="none-msg">
            <em>No executive actions match your specifications.</em>
        </p>
    {/if}
</main>
